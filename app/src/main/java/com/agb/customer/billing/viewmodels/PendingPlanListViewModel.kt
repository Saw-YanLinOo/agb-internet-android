...
class PendingPlanListViewModel : BaseViewModel() {

    //api call
    var myView: PendingPlanView?=null

    fun setView(mView: PendingPlanView){
        this.myView = mView
    }

    fun getPendingPlanList(request : com.agb.customer.billing.networks.requests.PaginationRequest){

        RestClient.getApiService().getPendingPlanList(request)
            .enqueue(object : Callback<PendingPlanListResponse> {

                override fun onFailure(call: Call<PendingPlanListResponse>, t: Throwable) {
                    myView!!.showError(Constants.CONNECTION_FAIL,"500")
                }

                override fun onResponse(
                    call: Call<PendingPlanListResponse>,
                    response: Response<PendingPlanListResponse>
                ) {
                    if (response.isSuccessful){
                        ...
                        when (mResponse.responseCode) {
                            ...
                            Constants.API_SUCCESS_CODE -> myView!!.setPendingPlanData(mResponse)
