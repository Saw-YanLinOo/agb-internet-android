...
import com.agb.customer.billing.networks.requests.CBPaymentQRDownloadRequest
...
import com.agb.customer.billing.views.CBPayInformationView
import retrofit2.Call
import retrofit2.Callback
import retrofit2.Response

class CBPayInformationViewModel : BaseViewModel() {

    //api call
    var myView: CBPayInformationView?=null

    fun setView(mView: CBPayInformationView){
        this.myView = mView
    }


    fun getCBPaymentQRDownload(request : CBPaymentQRDownloadRequest){

        RestClient.getApiService().cbPaymentQRDownload(request)
            .enqueue(object : Callback<CBPaymentQRDownloadQRResponse> {

                override fun onFailure(call: Call<CBPaymentQRDownloadQRResponse>, t: Throwable) {
                    myView!!.showError(Constants.CONNECTION_FAIL,"500")
                }

                override fun onResponse(
                    call: Call<CBPaymentQRDownloadQRResponse>,
                    response: Response<CBPaymentQRDownloadQRResponse>
                ) {
                    if (response.isSuccessful){
                        ...
                        when (mResponse.responseCode) {
                            ...
                            Constants.API_SUCCESS_CODE -> myView!!.setPaymentQRResponse(mResponse)
